...
if (!require("parallel", character.only = TRUE)) {
  install.packages("parallel", dependencies = TRUE, repos='http://cran.irsn.fr/')
  library(parallel)
}else{
  library(parallel)
}
if (!require("foreach", character.only = TRUE)) {
  install.packages("foreach", dependencies = TRUE, repos='http://cran.irsn.fr/')
  library(foreach)
}else{
  library(foreach)
}
if (!require("doParallel", character.only = TRUE)) {
  install.packages("doParallel", dependencies = TRUE, repos='http://cran.irsn.fr/')
  library(doParallel)
}else{
  library(doParallel)
}
...
do_subfolder <- function(aSubfolder,path_folder,path_output,nb_steps){
...
do_folder <- function(path_folder, path_output, nb_steps){
  dir.create(path_output,showWarnings = F,recursive = T)
  ...
  #clusterExport(cl, "subfolders")
  # clusterExport(cl, "path_folder")
  # clusterExport(cl, "path_output")
  # clusterExport(cl, "nb_steps")
  clusterExport(cl, "do_subfolder")
  clusterExport(cl, "get_file_info")
  clusterExport(cl, "INDIVIDUALS")
  clusterExport(cl, "BUILDINGS")
  clusterExport(cl, "INDIVIDUAL_VARIABLES")
  clusterExport(cl, "INDIVIDUAL_VARIABLES_NAMES")
  clusterExport(cl, "INDIVIDUAL_AGE_CATEGORIES")
  clusterExport(cl, "BUILDING_TYPES")
  #One subfolder is expected to contain the results of all simulations from a same experiment
  foreach(aSubfolder = subfolders)  %dopar%  
    do_subfolder(aSubfolder,path_folder,path_output,nb_steps)
  stopCluster(cl)
}
